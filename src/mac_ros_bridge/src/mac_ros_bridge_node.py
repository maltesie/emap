...
# This code is taken from TUBDAI's RHBP planning framework
...
class MacRosBridge(threading.Thread):
    ...
    def __init__(self, name=None):
        ...
        self._pub_request_action = rospy.Publisher('~request_action', RequestAction, queue_size=1, latch=False)
...
#            time.sleep(MacRosBridge.RETRY_DELAY)
    ...
    def _sim_start(self, message):
        ...
        msg.proximity = float(simulation.get('proximity'))# 1.0 / (10.0 ** int(simulation.get('proximity')))  # given proximity is converted to a more useful scale
        msg.cell_size = float(simulation.get('cellSize'))# / 1000.0  # convert from meter to kilometer
    ...
    def _send_action(self, action_type, params={}):
        ...
        rospy.logdebug("Action: " + str(eT.tostring(response)))
        #self._socket.send(b'store storage1 item1 A 1' + MacRosBridge.SEPARATOR)
    ...
    def _parse_products(self, simulation):
        ...
        for item in items:
            product = Product()
            product.name = str(item.get('name'))
            ...
            for role_item_elm in item:
                if role_item_elm.tag == "item":
                    consumed_item = Item()
                    consumed_item.name = str(role_item_elm.get('name'))
                    amount = role_item_elm.get('amount')
                    if amount:
                        consumed_item.amount = int(amount)
                    else:
                        consumed_item.amount = 1
                    consumed_items.append(consumed_item)
                elif role_item_elm.tag == "role":
                    roles.append(str(role_item_elm.get('name')))
                else:
                    rospy.logerr("Unknown item product '%s' ! %s", role_item_elm.tag, traceback.format_exc())
            product.consumed_items = consumed_items
    ...
    def _parse_resources(self, perception):
        ...
        for xml_item in perception.findall('resourceNode'):
            rospy.logdebug("Resource %s".format(str(eT.tostring(xml_item))))
            resource = Resource()
    ...
    def _parse_team(self, perception, timestamp):
        ...
        msg_team = Team()
        msg_team.massium = int(team.get('massium'))
        msg_team.score = int(team.get('score'))
    ...
    def _parse_agent(self, perception, timestamp):
        ...
        msg.known_resources = self._parse_resources(perception)
        msg.known_wells = self._parse_wells(perception)
    ...
    def _publish_request_action(self, timestamp, perception):
        ...
        msg.simulation_step = int(perception.find('simulation').get('step'))
        msg.deadline = int(perception.get('deadline'))
        ...
        msg.wells = self._parse_wells(perception)
        
        msg.charging_stations = self._parse_charging_stations(perception)
